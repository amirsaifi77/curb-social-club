...
| Signal | Why |
|---|---|
| Every slug starts `dev-`, every handle `dev_` | One `LIKE 'dev-%'` finds every fixture row |
...
| `events.csv.erb` | Seven meets, one per launch city, covering every cadence except `announced`, both host kinds plus the app account, and one sponsorship |
