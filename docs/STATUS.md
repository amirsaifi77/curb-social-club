...
| Session prompts | `docs/sessions.md` (preamble, template, Phase 0 sessions 0.1 to 0.10, Phase 1 sessions 1.1 to 1.17). Phase 0 is merged; Phase 1 sessions 1.1 (host migration and models), 1.2 (materializer, nightly job, describer), 1.3 (PostGIS list and map endpoints), 1.4 (event detail, occurrence reads, confirm, EventPolicy), 1.5 (decay, host consistency, venue dedupe), 1.6 (club, sponsor, and user host pages), 1.7 (sectioned feed, cross-resource search, venue search, sitemap), 1.8 (admin CRUD for venues, events, occurrences, and sponsorships), and 1.9 (admin CRUD for clubs, memberships, sponsors, users, and the dashboard) are done, next is 1.10 (seed importers, CSV seeds, and the first twenty verified meets) |
