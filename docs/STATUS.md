...
| Screen inventory | `docs/screens.md` (S01 to S41 plus S04b, W01 to W17, A01 to A12) |
| Session prompts | `docs/sessions.md` (preamble, template, Phase 0 sessions 0.1 to 0.10, Phase 1 sessions 1.1 to 1.17). Phase 0 is merged; Phase 1 sessions 1.1 (host migration and models), 1.2 (materializer, nightly job, describer), 1.3 (PostGIS list and map endpoints), 1.4 (event detail, occurrence reads, confirm, EventPolicy), 1.5 (decay, host consistency, venue dedupe), 1.6 (club, sponsor, and user host pages), 1.7 (sectioned feed, cross-resource search, venue search, sitemap), 1.8 (admin CRUD for venues, events, occurrences, and sponsorships), 1.9 (admin CRUD for clubs, memberships, sponsors, users, and the dashboard), 1.10 (seed importers, the A07 import screen, and the seed CSV headers), 1.11 (mobile onboarding and the sectioned Home feed), 1.12 (mobile Map with clustering, filter chips, bottom sheet, and List), 1.13 (mobile search), 1.14 (mobile event detail), 1.15 (mobile club, sponsor, and read-only profile pages), 1.16 (web home, meets list, event and occurrence pages, OG cards), and 1.17 (web host pages, city pages, map, sitemap, robots, AASA, error pages) are done: Phase 1 is code complete. What remains before a beta is the twenty verified seed rows, a deployed staging API, the confirmed domain, and the on-device passes recorded in each session's stopping note. The twenty verified meet rows are still to be written by hand into `apps/api/db/seeds/events.csv`: the importer, the fixtures, and the screen are ready for them. Device verification of 1.11 to 1.15 (the S01 permission flow, AC-8 to AC-12, AC-15 to AC-18, AC-20 to AC-22, the event detail ACs, and the host page ACs on an iPhone, plus the brand previews) waits on staging and those rows. AC-18 (universal links) additionally needs the AASA file on the domain, which lands with 1.17 |
...
0. Run the app on the iOS Simulator, which needs nothing that is still outstanding: `docs/local-development.md` is the recipe, and `bin/rails seeds:dev` fills a local database with fabricated meets so the screens are not empty. Only Sign in with Apple, universal links, and push still need a device, a domain, or a real build.
