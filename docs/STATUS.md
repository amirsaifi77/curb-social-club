...
| Session prompts | `docs/sessions.md` (preamble, template, Phase 0 sessions 0.1 to 0.10, Phase 1 sessions 1.1 to 1.17). Phase 0 is merged; Phase 1 sessions 1.1 (host migration and models), 1.2 (materializer, nightly job, describer), 1.3 (PostGIS list and map endpoints), 1.4 (event detail, occurrence reads, confirm, EventPolicy), 1.5 (decay, host consistency, venue dedupe), 1.6 (club, sponsor, and user host pages), 1.7 (sectioned feed, cross-resource search, venue search, sitemap), 1.8 (admin CRUD for venues, events, occurrences, and sponsorships), 1.9 (admin CRUD for clubs, memberships, sponsors, users, and the dashboard), 1.10 (seed importers, the A07 import screen, and the seed CSV headers), and 1.11 (mobile onboarding and the sectioned Home feed) are done, next is 1.12 (mobile Map with clustering, filter chips, bottom sheet, and List). The twenty verified meet rows are still to be written by hand into `apps/api/db/seeds/events.csv`: the importer, the fixtures, and the screen are ready for them. Device verification of 1.11 (the S01 permission flow, AC-8 to AC-12 on an iPhone, and the brand previews) waits on staging and those rows |
