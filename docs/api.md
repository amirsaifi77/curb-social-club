...
|---|---|---|---|
| GET | `/events` | anon | `near=lat,lng` and `radius_km` (default 32, which is the "20 miles" in product copy; max 160) or `bbox=w,s,e,n`; `from`, `to` (default now to +14 days); `tags[]`; `recurring=true`; `q`; `host=<type>:<id>` (for host pages); `sponsor=<id>` (events the sponsor hosts or backs); `sort=date|distance`; `past=true` (host, sponsor, and search lists only, ignored with `near` or `bbox`); cursor pagination. Returns EventSummary with `next_occurrence` inside the window. `q` without `near` or `bbox` searches everywhere; `q` with `near` defaults `radius_km` to 80. `bbox` with `near` filters by the box and returns `distance_m` from `near` (the map sheet). Without `near` and `bbox` the window has no upper bound unless `to` is sent, so a host page shows the next date however far out it is. Dormant and hidden events are excluded. |
...
| GET | `/sponsors/:slug` | anon | Sponsor. 404 when hidden unless the viewer is a platform admin, mirroring the club rule. |
...
| Method | Path | Auth | Notes |
|---|---|---|---|
| GET | `/feed` | anon | `near` param or device home location. Returns `{ data: { sections: FeedSection[] }, meta: { generated_at } }`. Each FeedSection is `{ kind, title, items, more: { path, params } | null }`. Section kinds and item shapes, in display order: `this_weekend` (EventSummary), `following` (mixed `{ type: "event" | "post", item }`, omitted for anonymous), `recent_photos` (Post, `photo` and `instagram` kinds, last 7 days within radius), `clubs_nearby` (ClubSummary, up to 6), `sponsors_nearby` (SponsorSummary with an upcoming hosted or sponsored event within radius, up to 4, organic and unpaid at launch), `spots_nearby` (SpotSummary, up to 6, Phase 4), `next_week` and `later` (EventSummary). Windows are computed in the venue's timezone, per row, so they do not shift when the server clock crosses midnight UTC: `this_weekend` is now through the coming Sunday 23:59, `next_week` is the following Monday through Sunday, `later` is the rest of the materialized horizon. Each of the three event windows holds at most ten items; `more` carries `near`, `radius_km`, and an ISO 8601 `from` and `to` that bound the section, so following it returns a superset of the section, never a subset. Empty sections are omitted. Ranking inside a section is by time or distance; there is no cross-section ranking at launch. Cached 60 s per rounded origin, radius, and viewer (one shared entry for anonymous callers, one per signed-in user, so a viewer specific section can never be served to someone else); the response is `public` for anonymous callers and `no-store` for signed-in ones. |
...
| Method | Path | Auth | Notes |
|---|---|---|---|
| GET | `/venues/search` | anon | `q` (required, 400 `bad_request` when blank), `near` as lat,lng. Returns `{ data: { venues: Venue[], suggestions: VenueSuggestion[] } }`, at most five of each. `venues` are rows we already have, matched on name by trigram similarity, nearest first when `near` is present and by similarity otherwise. `suggestions` are places the provider knows and we do not, as `{ name, address, lat, lng, external_place_id, external_source }`; nothing is stored until a host picks one. Queries under three characters skip the provider. A successful provider result, empty included, is cached 24 h per query and rounded origin; a provider failure is not cached and costs suggestions rather than the response. |
