...
- R-9 Every route under `/admin` except `GET /admin/sign_in` and `POST /admin/session` MUST redirect anonymous and `member` sessions to `/admin/sign_in` with 302; a `moderator` MUST see A02, A08 (list, show, suspend and unsuspend; no role change and no delete), A09, A10, and MUST get 302 to `/admin` with the flash in Copy on everything else; `admin` sees everything. (US-8)
...
- R-15 A04 MUST list events with filters `status`, `host_type`, `claimed` (yes or no), `stale`, `dormant`, and `q`, and edit `title`, `slug` (locked once published), `description`, a host picker by name that writes `host_type`, `host_id`, and `host_name` together from one control, so the three can never disagree, `venue_id` via a venue picker by name, `cadence`, `dtstart` (entered local in the event `timezone`), `duration_minutes`, `timezone`, `rrule`, `rrule_until`, `tags`, `status`, `visibility`, `source_url`, `source_type`, `external_host_name`, `capacity`, `rsvp_mode`, `verification_source_url`, `verified_at`, `last_confirmed_at`, the cover attachment, and nested `event_sponsorships` (sponsor picker, `role`, `note`, `position`). Locked fields (the host on a claimed event, the slug on a published one) MUST be dropped in the controller and MUST be audited as `skipped_locked_fields` only when the submitted value differs from the stored one, so a form that round-trips a disabled field writes no row. (US-2)
...
- R-18 A05 MUST edit `name`, `slug`, `description`, `avatar`, `banner`, `home_lat` and `home_lng` (to `home_location`), `home_label`, `links` (the six keys in `docs/data-model.md`, validated on write by the same rules as `profiles.links`), `join_policy`, `status` (`active`, `hidden`), `verified`, and `/admin/clubs/:id/memberships` MUST list memberships and add one by handle with `role`, change `role`, and remove, with the model's single-owner rule surfacing as a form error. Changing a member to `owner` MUST transfer ownership in one transaction, demoting the sitting owner to `admin`, because that is the path the owner error tells the admin to take and the model rejects a second owner row. A handle that does not resolve to an active user MUST be a form error on both the owner field and the membership form, never a silent fall back to the app account. (US-2)
...
- R-20 A07 MUST accept one file and a `kind` in `venues`, `clubs`, `sponsors`, `events`, store the upload as an Active Storage blob, render a dry-run preview (row number, natural key, `action`, resolved venue action, errors) without writing, then apply on a second submit that passes the blob's signed id, and render a results page with the same columns; files over 500 rows MUST be rejected with the copy below. The `kind` MUST travel with the blob rather than the second form, and the signed id MUST be refused unless it names a blob this screen stored, so Apply cannot run a different importer or a file the preview never read. A file that cannot be parsed at all (over the row limit, malformed CSV, not UTF-8) MUST render its message on the page it came from rather than raising. (US-3)
...
- R-22 A08 MUST list users with search on `handle` and `email`, show identities, session count, role, status, hosted events, and reports, and allow `role` change (`admin` only, never on oneself), suspend and unsuspend (`admin` and `moderator`, but only an `admin` may suspend a user whose role is `admin` or `moderator`, and nobody may suspend or delete their own account), and delete (`admin` only, confirm prompt). The reports column arrives with A10 in Phase 2; until then A08 shows identities, sessions, role, status, and hosted events. (US-2)
...
- R-26 Every list MUST use the shared partial `admin/shared/_table.html.erb` (columns, rows, empty copy) and every form the shared `_form_errors` and `_field` partials; destructive buttons use `admin.js` event delegation on `data-confirm`, no inline scripts. A list whose rows carry their own forms (A04 occurrences, A05 memberships) renders its own table instead, because the shared partial takes finished cells. (US-2)
...
None in v1. Admin routes are Rails resources under `namespace :admin`: `GET /admin/sign_in`, `POST /admin/session`, `DELETE /admin/session`, `GET /admin`, `resources :venues, :events, :clubs, :sponsors, :users, :spots`, nested `events/:id/occurrences`, `clubs/:id/memberships`, `POST /admin/events/:id/verify`, `POST /admin/events/:id/confirm`, `POST /admin/events/:id/rematerialize`, `POST /admin/clubs/:id/{hide,unhide,verify,unverify}` and the same four on `sponsors`, `PATCH /admin/users/:id/role`, `POST /admin/users/:id/suspend`, `POST /admin/users/:id/unsuspend`, `GET|POST /admin/seeds`, `POST /admin/seeds/apply`, `resources :claims` with `approve` and `reject`, `resources :reports` with `actions`, `POST /admin/spots/:id/merge`, `mount MissionControl::Jobs::Engine => "/admin/jobs"`. The public API's `POST /events/:id/confirm` remains the app-side path (events-and-occurrences.md R-24).
...
| AC-10 | A claimed event | The edit form is rendered | The host picker is disabled with the claimed copy; a crafted POST changing the host is ignored, and audited as `skipped_locked_fields` when the edit it came with saves | R-15 |
...
- `config.api_only` drops `new` and `edit` from the default resource actions, so every admin `resources` line names its actions with `only:` or those two HTML routes silently do not exist.
- A05 and A06 hide rather than delete: a hidden club or sponsor keeps hosting its events (clubs R-5, sponsors R-5), so neither screen has a delete.
