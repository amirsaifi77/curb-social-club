...
- R-12 The when block MUST show the next occurrence in the venue timezone, `rrule_text` for recurring events, the next four dates as rows that open S09, and an "Add to calendar" action that writes one calendar event (recurring rule included) through `expo-calendar` after permission. The rows are the dates after the one shown above them, and the block is absent when there are none: the same date twice on one screen reads as two meets. `expo-calendar` takes a structured recurrence rather than an RRULE string and cannot express the BY parts at all, so a rule whose meaning depends on one is not translated and writes a single event instead: dropping the part would put the meet on the wrong days, and "first Saturday of the month" becoming "the 3rd of every month" is worse than one entry. A weekly rule with a single BYDAY is the exception, since it repeats on the day the meet already starts. Refusing permission has a line; a calendar that cannot be written adds nothing and says nothing, since the person's settings are not the problem. (US-1, US-2)
