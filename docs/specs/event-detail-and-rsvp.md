...
- R-12 The when block MUST show the next occurrence in the venue timezone, `rrule_text` for recurring events, the next four dates as rows that open S09, and an "Add to calendar" action that writes one calendar event (recurring rule included) through `expo-calendar` after permission. The rows are the dates after the one shown above them, and the block is absent when there are none: the same date twice on one screen reads as two meets. `expo-calendar` takes a structured recurrence rather than an RRULE string. The API's grammar (`recurrence/rrule_validator.rb`) emits exactly two shapes, `FREQ=WEEKLY;BYDAY=<day>` and `FREQ=MONTHLY;BYDAY=<ordinal><day>`, never `UNTIL` or `COUNT`, and both translate: `daysOfTheWeek` carries a `weekNumber`, which is what an ordinal BYDAY means. A rule outside that grammar is not translated and writes a single event instead, since dropping the part it depends on would put the meet on the wrong days. A `seasonal` series is also written once: it ends on `events.rrule_until`, which the Event payload does not carry and the grammar cannot express, so the alternative is an entry repeating in someone's calendar for years. Refusing permission has a line; a calendar that cannot be written adds nothing and says nothing, since the person's settings are not the problem. (US-1, US-2)
...
- R-16 The going block MUST show `going_preview` avatars, "N going" and "M interested" counts, and the primary CTA "I'm going" using the states in `docs/components/primary-cta.md`, with a secondary "Interested" toggle; the counts row opens S10. In Phase 1 the block is counts only, and the Event payload carries no interested count or going preview (those are on Occurrence), so it shows "N going." alone rather than a hardcoded zero; the full line arrives with the RSVP slice, which is when Interested becomes settable. (US-4, US-5)
...
- R-20 A 410 or 404 response MUST render the "no longer listed" page with the nearby meets returned in the body, keeping the navigation chrome. The nearby meets arrive in the error's `details.nearby`, and a row the card cannot draw is dropped rather than taking down a page whose whole job is to be a soft landing. The status is read structurally, not by class: an error crossing a module boundary keeps its status and details but not always its prototype. (US-1)
- R-21 S09 MUST show one occurrence with its date, override note, counts, going preview, and the same CTA, and MUST mark past occurrences with "Ended". The Phase 1 stub is the date, the meet's title, the override note and "Ended", read from `GET /occurrences/:id` rather than passed in, so a `curb://occurrences/:id` link lands on the same screen as a "Next dates" row. (US-4)
- R-22 S10 MUST list attendees in two groups with vehicle lines ("Bringing a 1987 911") and MUST paginate. (US-5)
- R-23 Share MUST open the system share sheet with the canonical URL `https://curbsocial.club/meets/:slug` (plus `?token=` for unlisted events), MUST share the event URL when the current occurrence is cancelled, and MUST offer "Share to story" opening S34. The Copy table's message already ends with that URL, so the sheet is handed the message alone: passing a separate `url` gives iOS two activity items and the link lands in the sent text twice. (US-6)
...
- R-25 Universal Links and `curb://meets/:slug` and `curb://occurrences/:id` MUST open S08 or S09 directly, with the loading skeleton until the fetch resolves. The skeleton matches the layout (hero, then block lines), never a spinner or a line of text, since a deep link lands here with nothing on screen. (US-1)
