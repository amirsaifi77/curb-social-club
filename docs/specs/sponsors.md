...
- R-12 `GET /feed` MUST include a `sponsors_nearby` section only when at least one active sponsor within the browse radius hosts or is attached to a scheduled occurrence that is itself within the browse radius and inside the materialized horizon, MUST cap it at four SponsorSummary items ordered by soonest such occurrence then distance, and MUST omit it otherwise. A sponsor whose only upcoming meet is outside the radius is not near you and does not appear. (US-3)
...
- Adopted 2026-09-08 (session 1.15) into this spec: R-15's "See all" link opens the mobile route `meets?sponsor=<id>` (docs/screens.md S04), not the map's own list, which answers a viewport rather than a host. Phase 1 renders no Follow control on S14 at all, for the reason clubs.md R-15 gives.
