...
- R-8 The appearance setting MUST support `system`, `light`, and `dark`: `system` keeps adaptive themes on and calls `Appearance.setColorScheme("unspecified")` (React Native 0.86 renamed the old `null` argument); `light` or `dark` turns adaptive themes off, calls `UnistylesRuntime.setTheme`, and calls `Appearance.setColorScheme` with the same value so the native tab bar, sheets, and alerts follow the override. (US-2, US-3)
...
| AC-8 | Vitest with fake timers around the `useAsyncAction` machine in `packages/ui` | `fn` resolves at 100 ms | Status goes `idle` to `confirmed` with no `loading`; after 600 ms status is `going` | R-17 |
...
| Shared hook | `pnpm --filter @curb/ui test` (Vitest: `asyncActionMachine.test.ts` for AC-8 to AC-10 against the React-free machine, `useAsyncAction.test.tsx` for the hook) |
| Mobile unit | `pnpm --filter @curb/mobile test` (Jest: `PrimaryButton.test.tsx` snapshots for AC-11 and AC-12, plus the uncontrolled flow, the long-running bar, and stage copy for AC-14) and `pnpm --filter @curb/mobile lint` for AC-13 |
...
| 4 (Phase 0) | `useAsyncAction` in `packages/ui` with Vitest timing tests; `PrimaryButton` on mobile with Reanimated, SVG ring and check, haptics, reduced motion; snapshot tests; `dev/gallery` | R-16 to R-19 | AC-8 to AC-12, AC-14, AC-15 |
