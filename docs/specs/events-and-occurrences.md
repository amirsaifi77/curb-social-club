...
- R-11 The materializer MUST skip rows with `overridden_at` set, MUST NOT create rows for `announced`, `draft`, `cancelled`, or dormant events, MUST create exactly one row for `once`, and MUST set future `scheduled` rows that the current rule no longer produces to `cancelled` rather than deleting them. (US-3, US-4)
