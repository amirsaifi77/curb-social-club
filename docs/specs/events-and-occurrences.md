...
- R-11 The materializer MUST skip rows with `overridden_at` set, MUST NOT create rows for `announced`, `draft`, `cancelled`, or dormant events, MUST create exactly one row for `once`, and MUST set future `scheduled` rows that the current rule no longer produces to `cancelled` rather than deleting them. An overridden row owns its local calendar day in the event's timezone: the materializer neither inserts the rule's start for that day nor keeps a non-overridden row on it, and moving an override to another day does not cancel the original date. (US-3, US-4)
...
- R-18 `GET /events` without `near` or `bbox` but with `host=<type>:<id>`, `sponsor=<id>`, or `q` MUST query `events` directly, include `announced` events (and only those) with `next_occurrence` null, and order by `next_occurrence.starts_at` nulls last; an event whose dates have all passed belongs to the `past=true` list, not the upcoming one. This path has no upper window bound unless `to` is sent, so a monthly or seasonal series still shows its next date on a host page. (US-4)
...
- R-22 `GET /events/:slug` MUST return the Event shape with `upcoming_occurrences` (next 4 `scheduled` or `cancelled`), `dormant`, 404 for `draft` unless the viewer can edit, and 404 for `unlisted` unless `token` matches or the viewer can edit; `GET /events/:id/occurrences` MUST return upcoming occurrences including `cancelled` ones, paginated; `GET /occurrences/:id` MUST return the Occurrence shape with `timezone` and `override_note`. Both MUST 404 when the viewer cannot see the event's page, except that an `unlisted` event's dates stay readable by id, since the share link opens the page and the page loads them. (US-3)
...
- R-28 `dormant_at` MUST be cleared by `POST /events/:id/confirm`, by claim approval (admin.md A09), and by an admin "Verify now" or schedule edit (admin.md A04); the seed importer MUST set `last_confirmed_at` and `verified_at` to `verified_date` and MUST move both forward only, so re-running an older copy of a file changes neither and reports the row `skip`. (US-5, US-7)

**Seeds and jobs**

- R-29 `Seeds::EventRowImporter` MUST accept the CSV format in Data, validate every row before writing any (dry run returns a per-row report with `action` in `create`, `update`, `skip`, `error`), upsert on `slug`, resolve `host_type` and `host_slug` (blank means the app account as `User`), dedupe the venue per R-6, create `event_sponsorships` from `sponsors`, and never overwrite `host_*` or `claimed_at` on a claimed event (report `skip` for those columns). A row that fails validation is reported and left out; the rows around it are still written, so one bad row never blocks a file. Validation MUST cover the venue columns and MUST reject a `slug` or `source_url` repeated inside one file, naming the row it clashes with, so a preview that reports no errors is a preview whose apply cannot fail. The `sponsors` column is the whole truth for a row: emptying it detaches every sponsor, and a row whose only change is its sponsors or its venue is an `update`, not a `skip`. (US-7)
