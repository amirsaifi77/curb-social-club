...
- Adopted 2026-09-08 (session 1.15) into this spec: the Copy table names no header for the meets a host runs and no line for a host with none, so the Phase 1 S11 borrows the "Upcoming" header the other two host pages use and omits the block when there are no meets, rather than writing a sentence this spec did not. The Clubs section reads `GET /users/:handle/clubs` rather than the Profile shape's `clubs`, because only that endpoint sets the `role` the Owner and Admin labels need. Phase 1 renders no Follow control, no tabs, no garage and no overflow, which is Scope as written; the socials row and the blocked state (R-14) do ship.
