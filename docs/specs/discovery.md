...
- R-6 `this_weekend` MUST contain scheduled occurrences from now through the end of the coming Sunday in the venue's timezone; `next_week` MUST contain the following Monday through Sunday; `later` MUST contain the rest of the materialized horizon (90 days); each event appears once, by its next occurrence, ordered by `starts_at` then `distance_m`. Both the occurrence's day and "today" MUST be read in the venue's timezone, not the server's, so the sections do not shift when the server clock crosses midnight UTC. Each of the three windows MUST hold at most ten items and MUST carry a `more` link to `GET /events` with `near`, `radius_km`, and an ISO 8601 `from` and `to` that bound the section; because a section is a range of local calendar days, the link MAY return a superset of the section and MUST NOT return a subset. (US-2)
...
- R-13a `MapPin` carries `recurring` (`cadence <> 'once'`). S03 draws a series in its own pin style and a pin is all the map has, so the flag is on the pin rather than joined from the sheet's list, which is paginated and shorter than the 500-pin cap. (US-3)
...
- R-15 S03 MUST fetch `GET /events/map` for the visible bbox 300 ms after the region settles on first load and on tap of the "search this area" pill, MUST show the pill after the map moves more than 20 percent of the viewport or one zoom level from the last fetched box, and MUST cluster pins client-side with supercluster (radius 56 px, max zoom 16). The 56 px is screen pixels, so the index MUST set supercluster's `extent` to the same 256 px tile the zoom is read from: at supercluster's default extent of 512 the radius means 28 px on screen and pins stay apart where this rule says they cluster. A box wider than the API's 5 degrees is not sent at all; the sheet shows the zoom-in notice instead of spending a request on a 400, and shows nothing else, since that is not an empty area and the widen action could not help. While a new box is in flight the last box's pins stay on screen, dimmed, rather than blanking the map. (US-3)
- R-16 Tapping a cluster MUST zoom to its expansion zoom; tapping a pin MUST select it (1.2x, ring `textPrimary`) and scroll its card into view in the sheet; tapping a card MUST recenter the map on its pin and select it. The sheet is two screens at different detents: below the full detent it is S03's list of what is on the map and a card recenters and selects, at the full detent it is S04 and a card opens the meet. Pin style follows the pin, in this order: on now (started, within three hours), later today, then a series as `recurring` and anything else as `upcoming`. A MapPin carries `starts_at` and `recurring` but no end and no venue timezone, so "on now" is a window off the start and "today" is the reader's own day; on a map of meets near the reader those agree in all but the rarest case. A meet whose window has passed is not drawn, since `past` pins are out at launch (gaps item 30). (US-3)
...
- R-20 S05 MUST open from the glass search field on Home and Map, MUST debounce input 250 ms with a two-character minimum, MUST show up to ten recents when empty, MUST group results as Events, Clubs, Sponsors, Places (Spots after Places in Phase 4), and MUST move the map to a picked place and close. Clearing the field is not debounced: the recents come back at once. Recents are stored on the device only, most recent first, matched without regard to case, and never sent to the API. A group with nothing in it renders no title. Each API group is its own query, so a group that fails or is slow does not hold up the others; a group failing while another has rows is the offline state, and only every group failing is an error. Picking a place commits its coordinates through the R-1 rounding boundary, since the map is about to move there. Search results MUST NOT be written to the persisted cache: they are keyed by the query, and a browse response on disk is one thing while a search history on disk is another. S05's offline state is therefore the session's cache, not a relaunch's. Clearing the search history MUST clear the recents list and those cached searches together. A recent is recorded when the person acts on a search (submitting the field, or opening a result), never on every response, or a pause mid-word fills the ten slots with the prefixes of one word. Widening applies to one search and to the events query alone; the club and sponsor groups keep their `near`. A geocoder that cannot be reached is the offline state, not an empty Places group: an empty group would say the place does not exist. Places shows one result: the on-device geocoder returns coordinates without names, so several matches would render as identical rows and picking one would be a guess. A group still loading MUST NOT hide the groups that have answered. (US-5)
...
| S03 locate me failed | We couldn't find you just now. Pan the map instead. |
| S03 locate me label (VoiceOver) | Locate me |
| S03 loading | Looking for meets here. |
...
| S05 loading | Searching. (An inline line beside whatever has arrived, not a state that replaces the results.) |
