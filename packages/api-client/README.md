...
Typed client for the Cars and Coffee API, shared by web and mobile. Not implemented yet.

## Design

| Piece | Choice |
|---|---|
| Transport | `openapi-fetch` bound to the `paths` type from `@cac/types`, so every call is typed with zero runtime schema |
...
packages/api-client/
  ...
  tsconfig.json      # extends @cac/config/tsconfig.base.json
```

## Rules

Regenerate `@cac/types` before adding a hook for a new endpoint. No app-specific UI state in this package.
